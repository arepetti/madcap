...
class SessionStats:
    ...
    def record_confidence(self, label):
        """Bump the counter for a parsed verdict confidence label.

        `label` is one of "low" / "medium" / "high"; anything else is ignored
        (an unparseable verdict simply doesn't move the confidence counters).
        """
        if label in ("low", "medium", "high"):
            attr = f"verdict_confidence_{label}"
    ...
    def reset(self):
        ...
        self.verdict_confidence_low = 0
        self.verdict_confidence_medium = 0
        self.verdict_confidence_high = 0
