...
> **Implementation note.** This document describes the *design*. The high-level shape (rephrase → debate → verdict, the channel constraint, the answerer profile) is unchanged across implementations; the C# / .NET port additionally hardened two mechanisms that the original Python/Ollama version handled more loosely — the **inter-actor message contract** (now JSON rather than prose with magic-string markers; see below) and the **judge's internal structure** (now four single-job contexts rather than one persona). The current code runs on [Microsoft Foundry Local](https://learn.microsoft.com/azure/foundry-local/) (with an OpenAI-compatible remote backend as a config-driven alternative); the original Python/Ollama version is preserved under [old-python-src/](old-python-src). The model families differ per role for the reasons below; the .NET defaults are `phi-4` (Answerer), `mistral-7b-v0.2` (Critic), and `qwen3-8b` (Judge), which keep the "three distinct families" property the design calls for. They are configured in one place (`src/Debate.Cli/appsettings.json`). See [architecture.md](architecture.md) for the code map.
>
> **Message contract.** Every exchange between the pipeline and an actor is a JSON object with named fields, not free-form prose. The pipeline sends a prompt stating the exact JSON shape for that phase and parses a typed reply (e.g. `{"action":"rephrase"|"clarify","text":...}` for the rephrase step, `{"restatement":...}` for a restatement, `{"done":...,"objection":...}` for a critique, `{"answer":...,"confidence":...,"justification":...,"uncertainty":...}` for the verdict). This replaced an earlier protocol that keyed on prose markers like `REPHRASED:` / `CLARIFY:` / `PROFILE_NOTE:` / "no further objections" and regex-scraped a confidence label out of prose — fragile, locale-sensitive, and easy for a model to break by rephrasing. The JSON is parsed leniently (code fences and stray `<think>` blocks are stripped, the `{...}` span is isolated) and a single re-ask is issued on a parse failure. Note this structures the *envelope*, not yet the *content*: fields such as `restatement` and `objection` still carry free-form prose (see *Structured restatement to fight drift* under *Things to try next* for the next step).
...
- The judge sits between the answerer and the critic as a **one-way channel constraint**. After every answerer reply, the judge restates it as neutral facts (stripped of rhetoric, hedging, and persuasive framing), and that restatement is the *only* thing the critic ever sees. The answerer hears the critic raw; the critic only ever hears the answerer through the judge.
...
- Tries to answer the rephrased question directly and concisely, committing to one recommendation rather than listing options.
- **May ask for missing information on its first turn instead of guessing.** If an essential detail would change its recommendation, it can reply with a clarifying question rather than an answer; the judge rephraser relays it to the user and feeds the (rephrased) reply back. This happens *before* the debate and skips the critic — it is not a debate round. Once it has answered and the critic has objected, it defends or revises instead of asking.
...
- The orchestrator and ultimate arbiter. Plays several roles, on purpose. The judge is both an **LLM-as-a-Judge** [5] (issuing the verdict) and, at the debate-level, a **Meta-Judge** in the sense of Ma et al. [6] (mediating, normalising, and ruling on the exchange, not just scoring it).
- Those roles are driven through **separate, single-job contexts** (each its own conversation buffer, all backed by the same Judge model): a *rephraser*, a *restater* (the channel), an *arbiter* (the verdict), and a *profiler*. They share no memory with one another, so each sees only what its job needs — the rephraser never sees the debate, the arbiter never sees raw Answerer text, the profiler sees only the critic's objections. This is the "split the judge into specialised prompts" idea from *Things to try next*, made concrete; it does not change the Meta-Judge property from [6], which is about how many *models* contribute to the decision, not how many prompts one model is driven through.
- Runs three phases for every question:
  ...
  - **Phase 2 — mediate and verdict.** Mediates the debate by restating each answerer reply as neutral facts before the critic ever sees it (the channel constraint). At the end, given the critic's challenges as context, delivers the final answer in plain language with a low/medium/high confidence label and a note of any unresolved uncertainty.
...
1. **Setup.** The critic and all four judge contexts are dropped and rebuilt so their instructions reflect the latest session state (the rephraser's prior-exchange list; the critic's rephrased-question history and active answerer profile). The answerer is left alone and keeps its memory.
...
3. **Debate.** The answerer takes the rephrased question and replies — or, if essential information is missing, asks one clarifying question first; the rephraser puts it to the user and feeds the rephrased reply back to the answerer (bounded; this skips the critic and is not a round). Once it has an answer, a bounded loop runs:
  ...
  - if the critic isn't done, the answerer hears the critique directly and replies again, and the loop continues.
   The loop ends when the critic declares it's done or after the configured round cap (`MaxRounds`, default 3 — settable in config, the setup wizard, or `--rounds`).
4. **Verdict.** The arbiter — a fresh judge context that never ingested the raw answerer replies and never watched the critic argue live — is handed the exchange as a round-by-round transcript, each round pairing the judge's own neutral restatement of the answerer's position with the critic's objection to it. The answerer's rebuttal to an objection reaches the arbiter as the *next* round's restatement; an objection with no following round carries no rebuttal, which flags it as unanswered. The arbiter then produces the final answer, confidence label, and any remaining uncertainty. (Handing over a flat list of objections detached from the answers they were responding to was the earlier behaviour; threading rebuttals through the restatements lets the judge weigh rebutted vs. conceded objections instead of reconstructing the back-and-forth from memory.)
...
There is one deliberate exception to "the verdict is dropped" below: the judge's **rephraser** context keeps a running list of the prior rephrased questions *paired with the verdicts they reached*. This is the one place a past verdict survives, and it exists purely so the rephraser can resolve a follow-up question that refers back to an earlier topic ("make it faster", "what about that approach?") without the user re-specifying everything. It is never fed into the debate itself.

What is dropped at the start of every question:

- **The critic and the four judge contexts.** They're rebuilt from scratch so their freshly rendered instructions reflect the latest history and profile. The critic never carries memory of past debates; each judge context carries its own within-round memory but that memory does not persist across rounds (other than the rephraser's prior-exchange list described above).

What each judge context sees inside one round:
...
- **The rephraser** sees only the question (and the prior rephrased-question/verdict list), never the debate.
- **The restater** is the only context that ingests a raw answerer reply; it emits the neutral restatement and nothing downstream sees the raw text.
- **The arbiter** sees the debate in rephrased form only — its own restatements paired with the critic's objections, round by round. The answerer's rebuttal to an objection reaches it as the *next* round's restatement; an objection with no following round is flagged as unanswered.
- **The profiler** sees only the list of critic objections.
...
- **The verdict** is not stored as a structured artefact and is not fed back into the debate. It is generated by the arbiter, shown to the user, and otherwise dropped — with one exception: it is kept (as plain text, paired with its rephrased question) in the **rephraser's** session memory so follow-up questions can be resolved against earlier topics. In particular the answerer never sees the verdict: what gives continuity in the debate itself is the answerer remembering *its own replies and the critiques it received*, not the judge's final ruling.
...
sequenceDiagram
    ...
    Judge->>Judge: verdict, given the paired round-by-round exchange as context
...
- **A lossy, normalising channel.** The restatement is the only path from answerer to critic, and it is deliberately lossy: it keeps the substantive claims and drops the rhetorical surface. We call this a **channel constraint** rather than an "information bottleneck" on purpose. There is no information-bottleneck objective here, no learned encoder, and no rate-distortion trade-off being optimised; the judge is a prompted paraphraser, not a trained compressor. Earlier drafts borrowed the information-bottleneck label as a metaphor, but that framing oversells what is really just an extractive paraphraser whose job is to keep the *what* and drop the *how it was said*, so we have dropped it.
...
The profile is the only thing the system carries across rounds, and it is the part of the design we consider most genuinely novel. The broader ingredients are not new: persistent agent memory and self-reflection are well established (Reflexion [7] is the closest cousin — an agent stores verbal feedback about its own failures to do better next time), and giving an agent stack a structured view of who each agent is already exists in the persona-driven agent-simulation literature. What is unusual here is the *shape* of the memory rather than the fact of it. Three properties together set the profile apart from those precedents:

- **Third-party, not self-reflective.** The judge profiles the answerer; the answerer never reflects on itself. Reflexion-style memory is self-directed (the subject reasons about its own trajectory); here the observer and the subject are different actors.
- **Adversarially routed.** The learned weakness model is handed to the *opponent* — it is rendered into the critic's instructions to sharpen the next attack, not given back to the answerer to help it improve. This is closer to a red-team / coaching loop than to self-improvement.
- **Cross-question and trait-level.** It is not a per-task scratchpad; it is a slowly accumulated model of *how this reasoner tends to fail* that persists across unrelated questions and is meant to generalise.
...
- **Latency and token cost.** One round now costs (answer) + (restate) + (critique) + eventually (verdict). On local models this is annoying; on hosted APIs it is a real bill. We don't measure the gap against single-agent self-critique baselines like Self-Refine [18], and that gap is often small in the literature, so the extra cost has to be earned by the channel constraint and the profile.

## Things to try next

A few directions that follow naturally from the current shape.

- **Structured restatement to fight drift.** Actor replies are already JSON envelopes (see *Message contract* in the implementation note), but the restatement still travels as one free-form prose string inside `{"restatement": "..."}`, which is exactly where telephone-game errors creep in. The next step is to structure that *payload* — have the judge fill named fields instead of a paragraph, e.g.
  ```json
  {
    "core_claims": ["...", "..."],
    "assumptions": ["..."],
  ...
  ```
  The critic would then receive this structure instead of a prose blob. That makes "what is being claimed" auditable and makes drift between turns easy to detect mechanically: a claim that just disappears between turn N and turn N+1 is a clear flag.
- ~~**Split the judge into Mediator and Arbiter personas.**~~ *(Implemented.)* The judge is now driven through four specialised single-job prompts/contexts — rephraser, restater, arbiter, profiler — instead of one persona doing everything. Each has its own conversation buffer, so the contexts cannot leak into one another (the arbiter never sees raw answerer text, the rephraser never sees the debate, the profiler sees only the critic's objections). This went further than the original two-way Mediator/Arbiter split. It does not affect the Meta-Judge property from [6], which is about how many *models* contribute to the decision, not how many prompts a single model is driven through.
...
The judge is asked to rephrase the question, restate every answerer turn as neutral facts, and finally issue the verdict. Those three jobs have quite different objectives (neutralisation, information preservation, and decisive arbitration under conflict). The implementation now drives them through **separate single-job contexts** (rephraser / restater / arbiter / profiler) with no shared memory, which removes the crudest version of the problem — one buffer accumulating every job's output and degrading — and guarantees the arbiter sees the debate only in rephrased form. It does **not** remove the deeper issue: all four contexts are still the same model with the same biases, and the judge still writes the very surface (the restatement) that the verdict is then evaluated against — every answerer turn reaches the verdict only through the judge's own paraphrase. Structurally this is closer to a self-evaluation loop than to mediation, and it inherits self-preference bias [8]: even when the judge "is not told" who wrote what, perplexity-based familiarity cues can leak through and tilt the verdict toward outputs that match the judge's own style profile. The default model assignment mitigates the worst version of this by giving the Judge a different model from the Answerer (it shares the Critic's family instead); a Judge that ran the *same* model as the Answerer would be the maximal self-preference configuration. This is a partial defence, not a cure - familiarity cues are not eliminated by a model-family swap.
...
[7] Shinn, N., Cassano, F., Gopinath, A., Narasimhan, K., & Yao, S. (2023). *Reflexion: Language Agents with Verbal Reinforcement Learning.* arXiv:2303.11366 (NeurIPS 2023). An agent reflects on its own failed trajectories and stores verbal self-feedback in memory to do better on the next attempt. The closest cousin to the Answerer profile, and the contrast that defines it: Reflexion is self-directed (the agent reflects on itself) and aimed at improving that same agent, whereas the profile here is written by a third party (the judge) about the answerer and routed to the answerer's adversary (the critic).
